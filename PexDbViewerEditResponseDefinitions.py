...
class PexDbViewerEditResponseDefinitions(GeneratedGUI.EditResponseDefinitions):
	def __init__(self, parent, sqpfactory : sqp.SQFactory):
		...
		GeneratedGUI.EditResponseDefinitions.__init__(self, parent )
	...
	def get_dbdta2gui(self, active = True):
		...
		fact_q = sqp.SQQuery(self._fact, ResponseDefinition).where(ResponseDefinition.IsActive==active).order_by(ResponseDefinition.Name)
	...
	def m_newBUOnButtonClick( self, event ):
		newFact = ResponseDefinition(name="new result definition", abbreviation="_NEWRES")
